...
class FolderService:
    ...
    async def delete_folder(
        ...
        # Protect AI Artifacts folder from deletion (check by name since no metadata field)
        if folder.name.lower() == "ai artifacts" and folder.parent_id is None:
            raise ValueError(
                f"Cannot delete system folder: {folder.name}. "
                f"This folder is protected and managed by the system."
            )
    ...
    async def get_or_create_ai_artifacts_folder(
        self,
        db: AsyncSession,
        user_id: UUID
    ) -> Folder:
        """
        Get or create the AI Artifacts folder for a user.

        This folder stores files generated by the code interpreter.
        It's a system folder that cannot be deleted by users.
        ...
        Returns:
            Folder: The AI Artifacts folder
        """
        # Search for existing AI Artifacts folder (case-insensitive)
        stmt = select(Folder).where(
            Folder.user_id == user_id,
            func.lower(Folder.name) == 'ai artifacts',
            Folder.parent_id.is_(None)  # Root level only
        )
        result = await db.execute(stmt)
        existing_folder = result.scalar_one_or_none()

        if existing_folder:
            logger.debug(f"Found existing AI Artifacts folder for user {user_id}")
            return existing_folder

        # Create the folder
        logger.info(f"Creating AI Artifacts folder for user {user_id}")

        folder_data = FolderCreate(
            name="AI Artifacts",
            description="Files generated by AI code interpreter",
            parent_id=None
        )

        # Calculate path
        name_slug = "ai-artifacts"
        path = f"/{name_slug}"
        depth = 0

        # Create folder (without metadata field since it doesn't exist in model)
        folder = Folder(
            user_id=user_id,
            name=folder_data.name,
            description=folder_data.description,
        ...
        logger.info(
            f"Created AI Artifacts folder (id: {folder.id}) "
            f"for user {user_id}"
        )
